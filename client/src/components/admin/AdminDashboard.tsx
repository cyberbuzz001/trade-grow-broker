    ...
    <div className="flex flex-col gap-6 h-full overflow-y-auto pr-1">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            { label: 'Total Clients', value: kpis.customers.total, color: 'text-[var(--text-main)]', badge: 'All Accounts' },
            { label: 'Active Clients', value: kpis.customers.active, color: 'text-[var(--primary)]', badge: 'Trading Ready' },
            { label: 'New (30 Days)', value: kpis.customers.new, color: 'text-[var(--info)]', badge: 'Onboarded' },
            { label: 'KYC Pending', value: kpis.customers.kycPending, color: 'text-[var(--warning)]', badge: 'Action Required' },
            { label: 'KYC Rejected', value: kpis.customers.kycRejected, color: 'text-[var(--loss)]', badge: 'Failed Verification' },
            { label: 'Suspended', value: kpis.customers.suspended, color: 'text-[var(--loss)]', badge: 'Restricted' },
          ].map(k => (
            <div key={k.label} className="group bg-[var(--bg-surface)]/70 hover:bg-[var(--bg-surface)]/90 border border-[var(--border-color)]/80 hover:border-[var(--primary)]/40 rounded-xl p-3.5 transition-all duration-300 shadow-lg hover:-translate-y-0.5">
              ...
              <span className="text-[9px] font-semibold text-[var(--text-tertiary)] mt-1 block truncate">{k.badge}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            { label: 'Orders Today', value: kpis.trading.ordersToday.toLocaleString(), color: 'text-[var(--text-main)]' },
            { label: 'Executed Trades', value: kpis.trading.tradesToday.toLocaleString(), color: 'text-[var(--primary)]' },
            { label: 'Total Turnover', value: `₹${(kpis.trading.turnover / 100000).toFixed(2)}L`, color: 'text-[var(--gogrow-blue)]' },
            { label: 'Buy Turnover', value: `₹${(kpis.trading.buyValue / 100000).toFixed(2)}L`, color: 'text-[var(--primary)]' },
            { label: 'Sell Turnover', value: `₹${(kpis.trading.sellValue / 100000).toFixed(2)}L`, color: 'text-[var(--loss)]' },
            { label: 'Active Traders', value: kpis.trading.activeTraders.toLocaleString(), color: 'text-[var(--info)]' },
          ].map(k => (
            <div key={k.label} className="bg-[var(--bg-surface)]/70 hover:bg-[var(--bg-surface)]/90 border border-[var(--border-color)]/80 hover:border-[var(--info)]/40 rounded-xl p-3.5 transition-all duration-300 shadow-lg hover:-translate-y-0.5">
              <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider block">{k.label}</span>
              <span className={`text-2xl font-extrabold ${k.color} font-mono tracking-tight block mt-1`}>
                {k.value}
              </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'Total Account Funds', value: `₹${kpis.financial.totalFunds.toLocaleString('en-IN')}`, color: 'text-[var(--primary)]', desc: 'Pool Capital' },
              { label: 'Margin Utilized', value: `₹${kpis.financial.marginUtilized.toLocaleString('en-IN')}`, color: 'text-[var(--warning)]', desc: 'Blocked Margin' },
              { label: 'Brokerage Revenue', value: `₹${kpis.financial.brokerage.toLocaleString('en-IN')}`, color: 'text-[var(--gogrow-blue)]', desc: 'Zero Fee Active' },
              { label: 'Pending Withdrawals', value: kpis.financial.pendingWithdrawals, color: 'text-[var(--loss)]', desc: 'Fund Requests' },
            ].map(k => (
              <div key={k.label} className="bg-[var(--bg-surface)]/70 border border-[var(--border-color)]/80 hover:border-[var(--primary)]/40 rounded-xl p-4 transition-all duration-300 shadow-lg">
                <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider block">{k.label}</span>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'High-Risk Clients', value: kpis.risk.highRiskClients, color: 'text-[var(--loss)]', desc: 'Over-leveraged' },
              { label: 'Margin Call Alerts', value: kpis.risk.marginAlerts, color: 'text-[var(--warning)]', desc: 'Shortfall Warnings' },
              { label: 'RMS Order Blocks', value: kpis.risk.rmsBlocks, color: 'text-[var(--loss)]', desc: 'Pre-Trade Rejections' },
              { label: 'Frozen Accounts', value: kpis.risk.frozenAccounts, color: 'text-[var(--loss)]', desc: 'Suspended Access' },
            ].map(k => (
              <div key={k.label} className="bg-[var(--bg-surface)]/70 border border-[var(--border-color)]/80 hover:border-[var(--loss)]/40 rounded-xl p-4 transition-all duration-300 shadow-lg">
                <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider block">{k.label}</span>
                <span className={`text-xl font-extrabold ${k.color} font-mono tracking-tight block mt-1`}>{k.value}</span>
                <span className="text-[9px] font-medium text-[var(--text-tertiary)] block mt-1">{k.desc}</span>
              </div>
